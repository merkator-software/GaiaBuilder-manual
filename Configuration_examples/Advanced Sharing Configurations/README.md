...
- Each value must uniquely match with a connection string of a registered datasource on the target server.
...
6. **Configure environments**
   ...
   The semicolon (;) is included in database hints like 'demo_dev;' to handle edge cases, such as distinguishing between similar names like "user_dev" and "user_development", ensuring accurate matching with the target datasource connection string. It is required for postfix hints and optional otherwise.
...
<Details><Summary>List of the files stored in git on our environment</Summary>
   ...
   * 📄 `4f24b8ae0d6f4745b04b8d818dbf2e84.data.json`
   * 📄 `map.aprx.json`
   * 📄 `map.mapx.json`
   * 📄 `map.server.json`
   * 🖼️ `thumbnail.png`
   * 🖼️ `thumbnail_acc.png`
   * 🖼️ `thumbnail_dev.png`
   * 🖼️ `thumbnail_prod.png`
   * 🖼️ `thumbnail_test.png`
   * 📄 `acc.databasehint.json`
   * 📄 `acc.sharing.json`
   * 📄 `dev.databasehint.json`
   * 📄 `dev.sharing.json`
   * 📄 `prod.databasehint.json`
   * 📄 `prod.sharing.json`
   * 📄 `test.databasehint.json`
   * 📄 `test.sharing.json`  # Additional for advanced sharing
